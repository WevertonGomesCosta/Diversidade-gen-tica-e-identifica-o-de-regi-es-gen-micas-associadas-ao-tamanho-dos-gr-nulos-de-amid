...
BLUPS_join <- future_map_dfr(traits, ~ {
  data <- dados %>%
    select(1:4, all_of(.x)) %>%
    na.omit() %>%
    droplevels()
...
BLUPS_join %>%
  ...
  geom_vline(aes(xintercept = mean(Predicted), colour = "black"), linetype = "dashed") +
   geom_vline(aes(xintercept = quantile(Predicted, probs = 0.25), colour = "indianred4"), linetype = 2) +
   geom_vline(aes(xintercept = quantile(Predicted, probs = 0.75), colour = "indianred4"), linetype = 2 ) +
